...
UDlkCameraComponent::UDlkCameraComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer), CameraModeStack(nullptr)
{

}

void UDlkCameraComponent::OnRegister()
{
	Super::OnRegister();
	...
	if (!CameraModeStack)
	{
		...
		CameraModeStack = NewObject<UDlkCameraModeStack>(this);
	}
}

void UDlkCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	...
	FDlkCameraModeView CameraModeView;
...
void UDlkCameraComponent::UpdateCameraModes()
{
	check(CameraModeStack);
	...
	{
		if (TSubclassOf<UDlkCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
		{
