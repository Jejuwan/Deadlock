...
#include "SceneView.h"
#include "Deadlock/UI/IndicatorSystem/IndicatorDescriptor.h"
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UDlkIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UDlkIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
